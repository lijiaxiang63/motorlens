...
- Camera-only code (`src/camera/`, `src/tracking/cameraSource.ts`) is lazy-imported in `main.ts` and never loads in synthetic mode — it cannot be exercised headlessly; changes there need a real-camera check by the user. The same lazy-import rule covers `src/tracking/videoFile.ts` (dynamic import inside `ui/screens/videoReview.ts`): MediaPipe must stay out of the main bundle.
- Subject flow headlessly: home → "Open subjects" → create a subject → checklist "Start" runs record→results with a subject context; the results screen auto-saves to IndexedDB (assert via `indexedDB.open('motorlens')`, stores `subjects`/`results`/`videos`/`settings`) and `window.__lastReport.subject/source` are set. "Export all (ZIP)" sets `window.__lastExport` (Blob) before triggering the download. The upload path can be smoke-tested by recording a canvas `captureStream()` into a webm File and navigating `__ctx.navigate({name:'videoReview', subject, file})` — no hands detected means 0 auto-segments, but processing, review UI, manual segments, analysis, and persistence all execute (and MediaRecorder webm exercises the Infinity-duration workaround).
...
**Report round-trip as regression harness.** Results build a `SessionReport` (schemaVersion 1) with raw frames rounded to 4 dp; dropping an exported JSON on the home screen recomputes metrics from `raw.frames` and must reproduce the original numbers. The optional `subject`/`source` fields are additive — `parseSessionJson` only validates schemaVersion/app/frames, and `buildSessionReport` spreads them only when present, so pre-subject reports stay byte-identical (locked by `report/export.test.ts`).
